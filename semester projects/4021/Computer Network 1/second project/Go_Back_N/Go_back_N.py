...
while seq < totalFrames:
    ...
    if control == 0:  # Successful Transmission
        print(Fore.GREEN + f"RECEIVER: Received.... |(Frame{f}),(Seq{seq}),({data[seq]})|")
        ...
        if checkSum != sumValue:  # Checksum process
            ...
            print("RECEIVER: Checksum - False Positive.... Discard frame!")

        print(
            Fore.RED + f"SENDER's RECEIVER: Timeout!! |Ack{ack}| Not Received (Frame Corrupted)\n" + Style.RESET_ALL)

    elif control == 2:  # Frame lost case
        print(Fore.RED + f"RECEIVER: Not Received.... |(Frame{f}),(Seq{seq}),({data[seq]})|")
        ...
        print(Fore.RED + f"RECEIVER: Received.... |(Frame{f}),(Seq{seq}),({data[seq]})|")
